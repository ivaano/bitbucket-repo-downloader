...
def clone_repo_list(repo_list: list, dest_path: str):
    for repo in repo_list:
        dest_path = os.path.join(dest_path, repo.get('project').get('name'), repo.get('name'))
        if os.path.isdir(dest_path):
            try:
                update_repo(repo.get('links').get('clone')[1].get('href'), dest_path)
            except:
                logging.warning('Repo {} had some problems during cloning'.format(repo.get('name')))
                pass
        else:
            try:
                clone_repo(repo.get('links').get('clone')[1].get('href'), dest_path)
            except:
                logging.warning('Repo {} had some problems during cloning'.format(repo.get('name')))
                pass
...
def main(interactive: bool):
    ...
    if not interactive:
        ...
        text_fragments = FormattedText([("class:input", "bitbucket user: ")])
        user = prompt(text_fragments, style=style)
        text_fragments = FormattedText([("class:input", "bitbucket password: ")])
        password = prompt(text_fragments, style=style, is_password=True)
        try:
            bb = Client(user=user, password=password)
            workspaces = get_workspaces(bb)
            slugs = [s.get('slug') for s in workspaces if s.get('slug')]
            ...
            if workspace not in slugs:
                raise NotFoundError
            text_fragments = FormattedText([("class:input", "destination path: ")])
            destination_path = prompt(text_fragments, style=style)
            if not os.path.isdir(destination_path):
                raise DestinationPathError
        except NotAuthenticatedError:
            print = print_formatted_text
            ...
            print = print_formatted_text
            text_fragments = FormattedText([
                    ("class:warn", "Invalid Destination path, it must be a directory that exists. ")])
            print(text_fragments, style=style)
            exit(1)

    logging.info('Starting')
    repos = get_all_repos(bb, workspace)
    clone_repo_list(repos, destination_path)
    logging.info('End')
